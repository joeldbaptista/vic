/*
 * regex.c - regex engine: compiles a pattern to an instruction array and
 * matches it with a small VM that walks it (fork/jump/mark/match).
 * Vendored from neatvi's regex.c; see regex.h for the public API.
 *
 * Grammar is POSIX Extended (ERE) unconditionally: bare (), |, +, ? are
 * always special. REG_EXTENDED has no effect (kept for source
 * compatibility with existing call sites).
 *
 * Pipeline: rnode_parse() builds a tree from the pattern, rnode_emit()
 * lowers the tree to a struct regex program, re_rec() walks the program
 * with backtracking (rstate holds the saved-state stack). Subexpression
 * captures are recorded as "marks" during matching and folded into
 * rm_so/rm_eo pairs by rstate_marks().
 */
#include "regex.h"
...
/* regular expression atom types */
enum {
	RA_CHR = '\0', /* character literal */
	RA_BEG = '^',  /* string start */
	RA_END = '$',  /* string end */
	RA_ANY = '.',  /* any character */
	RA_BRK = '[',  /* bracket expression */
	RA_WBEG = '<', /* word start */
	RA_WEND = '>', /* word end */
};

/* regular expression tree node types */
enum {
	RN_ATOM = '\0', /* leaf atom */
	RN_CAT = 'c',   /* concatenation */
	RN_ALT = '|',   /* alternation */
	RN_GRP = '(',   /* capturing group */
};

/* compiled program instruction opcodes */
enum {
	RI_ATOM = '\0', /* match ra */
	RI_FORK = 'f',  /* fork execution to dst */
	RI_JUMP = 'j',  /* jump to dst */
	RI_MARK = 'm',  /* record the current position */
	RI_MATCH = 'q', /* pattern matched */
};
...
struct rinst {
	struct ratom ra; /* atom (RI_ATOM) */
	int ri;          /* opcode (RI_*) */
	int dst;         /* destination of RI_FORK, RI_JUMP */
	int mark;        /* mark number (RI_MARK) */
};
...
/* saved backtracking state */
struct rstate_saved {
	int s, mark_pos, pc; /* saved rstate state */
};

/* a recorded mark update */
struct rstate_mark {
	int num, val; /* mark number and byte offset */
};

/* matching state for one regexec() call */
struct rstate {
	...
	int mark_pos, mark_len;      /* used/capacity of mark[] */
	struct rstate_saved *saved;  /* saved backtracking states */
	int saved_pos, saved_len;    /* used/capacity of saved[] */
...
static void
rstate_init(struct rstate *rs, char *s, int flg, int subcnt)
{
	rs->o = s;
	rs->s = s;
	rs->flg = flg;
...
static int
rstate_push(struct rstate *rs, int pc)
{
...
static int
rstate_mark(struct rstate *rs, int mark)
{
	if (mark >= rs->subcnt * 2)
		return 0;
...
static void
rstate_marks(struct rstate *rs, regmatch_t sub[])
{
	int i;
...
static struct rnode *
rnode_make(int rn, struct rnode *c1, struct rnode *c2)
{
...
static void
ratom_copy(struct ratom *dst, struct ratom *src)
{
	dst->ra = src->ra;
	dst->s = NULL;
	if (src->s) {
...
static void
ratom_readbrk(struct ratom *ra, char **pat)
{
	int len = brk_len(*pat);
...
static void
ratom_read(struct ratom *ra, char **pat)
{
	char *s;
	int len;
...
static int
brk_match(char *brk, int c, int flg)
{
	int beg, end;
	size_t i;
	int neg = brk[0] == '^';
	char *p = neg ? brk + 1 : brk;
	char *p0 = p;
...
static int
ratom_match(struct ratom *ra, struct rstate *rs)
{
...
static int
rnode_count(struct rnode *rnode)
{
	int n = 1;
...
static int
rnode_grpnum(struct rnode *rnode, int num)
{
	int cur = 0;
...
static int
re_insert(struct regex *p, int ri)
{
	p->p[p->n++].ri = ri;
	return p->n - 1;
}
...
static void
rnode_emitnorep(struct rnode *n, struct regex *p)
{
	int fork, done, mark;
...
static void
rnode_emit(struct rnode *n, struct regex *p)
{
...
int
regcomp(regex_t *preg, const char *pat, int flg)
{
	/*
	 * Parsing only ever advances a cursor over pat, it never writes
	 * through it; the cast lets the internal parser keep a plain
	 * mutable char ** while the public API stays const-correct.
	 */
	char *cursor = (char *)pat;
	struct rnode *rnode = rnode_parse(&cursor);
	struct regex *re;
...
regfree(regex_t *preg)
{
	...
	if (!re) /* mirrors free(NULL); safe on a never-compiled regex_t */
		return;
	for (i = 0; i < re->n; i++)
...
static int
re_rec(struct regex *re, struct rstate *rs)
{
	struct rinst *ri;
...
static int
re_recmatch(struct regex *re, struct rstate *rs)
{
	rs->pc = 0;
	rs->mark_pos = 0;
	rs->saved_pos = 0;
...
int
regexec(const regex_t *preg, char *s, int nsub, regmatch_t psub[], int flg)
{
	struct regex *re = *preg;
	struct rstate rs;
	char *o;
	...
	/*
	 * do-while so a zero-length s (an empty line) still gets one
	 * attempt at its only position; equivalent to a pre-check while
	 * for non-empty s, since the loop condition re-tests the same o
	 * a pre-check would have tested before the next iteration.
	 */
	do {
		rs.s = o = s;
		s += uc_len(s);
